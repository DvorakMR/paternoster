...
with open('sample12.csv', 'r') as csvfile:
...
for i in range(len(data_in_percent)):
    ...
    elif data_in_percent[i] <= (quater_circle_perc+straight_percentage/2): #Straight up
        x = R
        y = R + (data_in_percent[i]-quater_circle_perc)*LENGTH
    elif data_in_percent[i] <= (quater_circle_perc+straight_percentage/2+ 2 * quater_circle_perc): #Half top circle
        x = R*np.cos((data_in_percent[i]-(quater_circle_perc+straight_percentage/2))*10*np.pi)
        y = R+(LENGTH*straight_percentage/2)+R*np.sin((data_in_percent[i]-(quater_circle_perc+straight_percentage/2))*10*np.pi)

    elif data_in_percent[i] <= (1-quater_circle_perc): #straight down
        x = -R
        y = (R + LENGTH*straight_percentage/2) - (data_in_percent[i]-((straight_percentage/2 + (3*quater_circle_perc))))*LENGTH
...
while running:
    ...
    # Update the screen
        ...
        if x_i%100 == 0:
        line1.set_xdata(x_plt[0:x_i])
        line1.set_ydata(y_plt[0:x_i])
        fig.canvas.draw()
